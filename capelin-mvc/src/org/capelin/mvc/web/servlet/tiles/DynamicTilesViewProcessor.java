...
public class DynamicTilesViewProcessor {
	...
	final Logger logger = LoggerFactory
			.getLogger(DynamicTilesViewProcessor.class);
	...
	private String defaultTilesDefinitionName = "mainTemplate";
	...
	protected void renderMergedOutputModel(String beanName, String url,
			...
			HttpServletResponse response, TilesContainer container)
			throws Exception {
		...
		String definitionName = startDynamicDefinition(beanName, url, request,
				response, container);

		container.render(definitionName, request, response);

		endDynamicDefinition(definitionName, beanName, request, response,
				container);
	}
	...
	protected String processTilesDefinitionName(String beanName,
			TilesContainer container, HttpServletRequest request,
		...
		if (derivedDefinitionName != null) {
			...
			if (lastIndex == -1) {
				...
				String path = (beanName != null ? beanName.substring(0,
						lastIndex) : "");
			...
			Object tileDef = (String) request
					.getAttribute(WebViewUtils.TILE_DEF);
			String tilesDefinitionName = (null == tileDef) ? defaultTilesDefinitionName
					: (String) tileDef;
