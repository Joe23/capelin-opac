...
public class TagUtils extends TagSupport {
	...
	public static String scl(String s) {
		...
		s = encode(s).replace(StaticStrings.DB_SPLIT_STRING,
				StaticStrings.SEMICOLON_SPACE);
		return s;
	}
	...
	public static String url(String s) {
		...
		for (String line : array) {
			if (line.startsWith("http://")) {
				sb.append("<a target=\"_blank\" href=\"").append(line)
						.append("\">").append(line).append("</a>");
	...
	public static String clink(String s) {
		...
		if (null != s) {
			...
			s = s.replaceAll("%", StaticStrings.SPACE);
			try {
				sb.append(WebViewUtils.U_DOUBLE_QUOTE)
						.append(URLEncoder.encode(s, "UTF8"))
						.append(WebViewUtils.U_DOUBLE_QUOTE);
			} catch (UnsupportedEncodingException e) {
				sb.append(WebViewUtils.U_DOUBLE_QUOTE).append(s)
						.append(WebViewUtils.U_DOUBLE_QUOTE);
			}
		}
